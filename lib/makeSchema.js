...
function tableShouldBeAtTopLevel(opts) {
    return {
        match(node) {
            return node.kind === 'block';
        },
        validate(block) {
            const invalid = block.filterDescendants(n => n.type === opts.typeTable);
            if (invalid.isEmpty()) return;
            return invalid;
        },
        normalize(transform, block, invalid) {
...
function cellsContainBlocks(opts) {
    return {
        ...
        normalize(transform, { nodes, key }) {
            const transformOptions = { normalize: false };
            transform = transform.wrapBlockByKey(nodes.first().key, opts.typeDefault, transformOptions);
            ...
            return nodes.rest().reduce(
              (tr, n, index) => tr.moveNodeByKey(n.key, wrapper.key, index + 1, transformOptions),
              transform
            );
        }
    };
