...
class TablePosition extends new Immutable.Record(DEFAULTS) {
  ...
  /**
     ...
     */
  static create (change, cell, opts = {}) {
    cell = findBlock(opts.typeCell, change, cell) || cell
    const row = findBlock(opts.typeRow, change, cell)
    const table = change.state.document.getParent(row.key)
